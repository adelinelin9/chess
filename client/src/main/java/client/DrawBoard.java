...
public class DrawBoard {
    public final static String WHITE_PERSPECTIVE = drawBoard();
    public final static String BLACK_PERSPECTIVE = drawBoardFlipped();
    ...
    public static String drawBoard() {
        ...
        for (int i = 0; i < board.length; i++) {
            boardString.append(SET_BG_COLOR_LIGHT_GREY).append(BOLD + BLACK + " ").append(8 - i)
                    .append(" ").append(RESET).append(RESET_BG_COLOR);
            for (String cell : board[i]) {
                boardString.append(cell);
            }
            boardString.append(SET_BG_COLOR_LIGHT_GREY).append(BOLD + BLACK).append(" ").append(8 - i)
                    .append(RESET).append(" ").append(RESET_BG_COLOR).append("\n");
        }
    ...
    public static String drawBoardFlipped() {
        ...
        for (int i = board.length - 1; i >= 0; i--) {
            boardString.append(SET_BG_COLOR_LIGHT_GREY).append(BOLD + BLACK + " ").append(8 - i).append(" ")
                    .append(RESET).append(RESET_BG_COLOR);
            for (int j = board[i].length - 1; j >= 0; j--) {
                boardString.append(board[i][j]);
            }
            boardString.append(SET_BG_COLOR_LIGHT_GREY).append(BOLD + BLACK).append(" ").append(8 - i).append(RESET)
    ...
    public static String[][] initializeBoard() {
        ...
        for (int i = 0; i < 8; i++) {
            board[1][i] = (i % 2 == 0 ? SET_BG_COLOR_BLACK : SET_BG_COLOR_WHITE) + BOLD + BLUE + BLACK_PAWN +
                    RESET_BOLD_FAINT + RESET_BG_COLOR;
        }
        ...
        for (int i = 0; i < 8; i++) {
            board[6][i] = (i % 2 == 0 ? SET_BG_COLOR_WHITE : SET_BG_COLOR_BLACK) + BOLD + RED + WHITE_PAWN +
                    RESET_BOLD_FAINT + RESET_BG_COLOR;
        }
        return board;
    }
